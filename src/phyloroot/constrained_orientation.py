...
def constrained_orientation_binary(network, root_edge, reticulations):
    ...
    while len(readyNodes) > 0:  # subdividing the root edge gives one additional edge
        ...
        if orientingNodeInDeg > 2 or (
            orientingNodeInDeg == 2 and orientingNode not in reticulations
        ):
            network.add_edges_from([root_edge])
            return False
        children = set(network.neighbors(orientingNode)) - set(
            diNetwork.predecessors(orientingNode)
        )
        newArcs = []
